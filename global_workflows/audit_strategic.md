...
3. **Decision Gate** — Apply the following logic:
   ...
   * **If adversarial agents are FOUND and `docs/governance/critiques/` is MISSING or EMPTY**:
     * 🛑 **STOP and ASK the user**. List the dynamically discovered agents, for example:
       ...
       >
       > **[Yes]** → I will generate the unified loop files by invoking each agent, then proceed with the audit using real data.
       > **[No]** → I will proceed with the audit in **Virtual Adversary** mode.
...
* **Consolidated Recommendations Matrix**:pro
