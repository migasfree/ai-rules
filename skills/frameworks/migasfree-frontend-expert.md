...
triggers:
  ...
  - "composition api"
  - "script setup"
dependencies: [migasfree-ui-ux-expert, security-expert]
---
...
- **Interaction Intent**: Brief description of the UX feel and behavior.
- **The Component (SFC)**: Clean, `<script setup>` compliant Code. Provide modular Vue components.
- **Organization**: Follow the `src/` hierarchy (components, pages, stores, composables).
- **Documentation**: Use JSDoc for exports.
- **Commit Messages**: Use the `[type](scope): description` format (e.g., `feat(ui): add new computer dashboard`).
